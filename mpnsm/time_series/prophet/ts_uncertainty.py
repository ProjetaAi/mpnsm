...
def add_prophet_uncertainty(
    ...
    using_train_df: bool = False) -> pd.DataFrame:
    """
    ...
    if (using_train_df) | (prophet_obj.growth == "flat"):  # there is no trend-based uncertainty if we're only looking on the past where trend is known
        sample_trends = np.zeros(len(forecast_df))
